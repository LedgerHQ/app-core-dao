...
This repository implements the Core protocol for **clear-signing** Bitcoin PSBTs whose structure is not supported by the vanilla Bitcoin app.

### Supported transaction types

| Type | Synonyms | Purpose in Core | Key characteristics |
|------|----------|--------------------|---------------------|
| **Lock** | “Stake” | Create a time-locked UTXO that represents the staked funds | Internal inputs only; ≤ 3 outputs (locking output ✚ Core `OP_RETURN` ✚ optional change) |
...
* A PSBT may contain any number of **Core inputs** (previous lock outputs).

#### 2. Lock (stake) transactions
* If the TX contains an `OP_RETURN` output, that output **must** be a valid Core metadata output **and** the TX **must** also contain a lock output.
* The PSBT may have **at most one** change output.
* If *any* input is a Core input, the outputs are limited to **change** and **lock** outputs only (i.e., no arbitrary payments).

#### 3. Unlock (unstake) transactions
* If the TX spends a CLTV (time-locked) Core UTXO, it **must** follow the Core unlock format.
...
By enforcing these constraints, the app guarantees that every signature produced on-device corresponds to a transaction that is valid under the Core staking model and cannot be mis-used for unintended transfers.
